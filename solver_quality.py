...
def _cargar_contexto_calidad(conn,i,p):
    ...
    try:
        return _cargar_asignaciones(cur,i,p),_cargar_slots(cur,i,p),_cargar_preferencias_periodo(cur,i,p),cargar_restricciones_docentes(cur,i,p)
    finally:cur.close()
...
def _optimizar_intercambios(i,p,asignaciones,slots,prefs,inicial,restricciones):
    ...
    while intentos<MAX_INTERCAMBIOS and sin<MAX_SIN_MEJORA:
        ...
        try:
            _validar_resultado_final(asignaciones,slots,cand)
...
def optimizar_horarios_institucion(institucion_id:int,periodo_lectivo_id:int,conn):
    ...
    try:
        ...
        asignaciones,slots,prefs,restricciones=_cargar_contexto_calidad(conn,institucion_id,periodo_lectivo_id)
        validar_horario_restricciones(asignaciones,slots,inicial,restricciones)
